...
单币种多边轧差清算全栈演示：录入义务 → 执行轧差 → 查看净头寸 → 确认 settle；并维护多币种汇率本，按结算日与目标币种输出折算净额报告。
...
8. 「汇率报告」页查看 seed 灌入的 6 个币种对汇率；选择结算日与目标币种生成折算净额报告
9. 以 operator 修改一条汇率（如 EUR→USD），报告数字随重新生成立即变化；viewer 只能浏览
10. 使用 `viewer` 登录，确认只能浏览、无法执行写操作

折算规则：同币种按 1 恒等折算；跨币种必须存在「结算日当天或之前生效」的汇率，缺失则报告失败并指出缺失币种对（如 `EUR->USD`），绝不默认按 1 折算。
